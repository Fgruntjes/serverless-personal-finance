...
namespace App.Function.Integration.Ynab.Tests.Controller
{
    public class StatusControllerTest : IntegrationTestFixture<Program>
    {
        ...
        {
            var getUserResponse = new YNAB.Rest.ApiResponse<UserData>
            {
                Data = new UserData
                {
            ...
            (await _client.GetFromJsonAsync<ApiResponse<IntegrationStatus>>("/status"))
                .Should()
                .BeEquivalentTo(new ApiResponse<IntegrationStatus>(new IntegrationStatus(true, getUserResponse.Data.Id)));
        }
        ...
        public async void VoidWhenNotConnected()
        {
            ...
            (await _client.GetFromJsonAsync<ApiResponse<IntegrationStatus>>("/status"))
                .Should()
                .BeEquivalentTo(new ApiResponse<IntegrationStatus>(new IntegrationStatus())
                {
